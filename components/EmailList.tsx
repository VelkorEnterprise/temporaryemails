    ...
    <div className="space-y-4">
      {messages.map((message) => (
        <div
          ...
          className="group p-4 cursor-pointer rounded-2xl border border-gray-100 hover:border-indigo-300 bg-white hover:bg-indigo-50/30 transition-all duration-300 ease-in-out hover:shadow-xl hover:shadow-indigo-500/10"
        >
          <div className="flex justify-between items-start gap-4">
            <div className="flex-shrink-0">
                <div className="w-12 h-12 bg-gray-50 rounded-xl flex items-center justify-center font-black text-lg text-gray-300 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-500 transform group-hover:rotate-6">
                    {message.from.name ? message.from.name[0].toUpperCase() : (message.from.address[0].toUpperCase())}
                </div>
            </div>
            <div className="flex-grow overflow-hidden">
                <div className="flex items-center justify-between mb-1">
                    <p className="font-black text-gray-900 truncate text-base group-hover:text-indigo-600 transition-colors tracking-tight">{message.from.name || message.from.address}</p>
                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex-shrink-0 bg-gray-50 px-2 py-1 rounded-full">{formatDate(message.createdAt)}</span>
                </div>
                <p className="text-gray-800 font-bold truncate text-sm mb-1">{message.subject || '(no subject)'}</p>
                <p className="text-gray-500 text-xs truncate leading-relaxed font-medium">{message.intro}</p>
            </div>
          </div>
        </div>
      ))}
